...
class build_FOL_graph:
    ...
    def build_graph(self,graph,prev,value,opera_sign):
        graph.add_nodes_from([item.lower().replace('¬','not ') for item in prev if len(item) >= 1])
        graph.add_nodes_from([item.lower().replace('¬','not ') for item in value if len(item) >= 1])
        if opera_sign == '∧':
            graph.add_edges_from([(itemx.lower().replace('¬','not '),itemy.lower().replace('¬','not '),{'type':1}) for itemx in prev for itemy in value if itemx != itemy and len(itemx)>=1 and len(itemy)>=1])
            graph.add_edges_from([(itemy.lower().replace('¬','not '),itemx.lower().replace('¬','not '),{'type':1}) for itemx in prev for itemy in value if itemx != itemy and len(itemx)>=1 and len(itemy)>=1])
        elif opera_sign == '→':
            graph.add_edges_from([(itemx.lower().replace('¬','not '),itemy.lower().replace('¬','not '),{'type':2}) for itemx in prev for itemy in value if itemx != itemy and len(itemx)>=1 and len(itemy)>=1])
        elif opera_sign == '∨':
            graph.add_edges_from([(itemx.lower().replace('¬','not '),itemy.lower().replace('¬','not '),{'type':3}) for itemx in prev for itemy in value if itemx != itemy and len(itemx)>=1 and len(itemy)>=1])
            graph.add_edges_from([(itemy.lower().replace('¬','not '),itemx.lower().replace('¬','not '),{'type':3}) for itemx in prev for itemy in value if itemx != itemy and len(itemx)>=1 and len(itemy)>=1])
    ...
    def graph2matrix(self,graph):
        ad_matrix = nx.to_numpy_matrix(graph,weight='type')
    ...
    # def graph2index_type(self,graph):
    #     edge_index = []
    #     edge_type = []
    #     word_nodes = graph.nodes
    #     for edge in graph.edge(data=True):
    #         start,end,attr = edge
    #         edge_index.append([start,end])
    #         edge_type.append(attr.get('type'))
    #     edge_index = np.array(edge_index, dtype=np.int64).T.copy()
    #     edge_type = np.array(edge_type, dtype=np.int64)
    #     return edge_index,edge_type,list(word_nodes)
...
class ABSADataset(Dataset):
    def __init__(self, fname, tokenizer,opt,tname):
        ...
        def adjacency_matrix_to_edge_index_type(adjacency_matrix,max_length=opt.edge_num):
            edge_indices = torch.nonzero((adjacency_matrix), as_tuple=False)
            if edge_indices.size(0) >= max_length:
                edge_indices = edge_indices[edge_indices.size(0)-max_length:,:]
                edge_values = adjacency_matrix[edge_indices[:, 0], edge_indices[:, 1]] - 1
                edge_index = edge_indices.t()
                # edge_index = edge_indices
            else:
                edge_values = adjacency_matrix[edge_indices[:, 0], edge_indices[:, 1]] - 1
                edge_indices = torch.concat((edge_indices,torch.tensor([[0,0]]*(max_length-edge_indices.size(0))))).long()
                # edge_index = edge_indices
                edge_index = edge_indices.t()
                # print(edge_values)
                edge_values = torch.concat((edge_values,torch.tensor([-1]*(max_length-edge_values.size(0))))).long()
            # print(adjacency_matrix,edge_index, edge_values)
            return edge_index, edge_values
        
        def Rgraph_PreModel(graph_matrix_text_list,max_length=opt.nodes_num):
            graph_matrix,text_list = graph_matrix_text_list
            np.fill_diagonal(graph_matrix, 4)
            _graph_matrix = graph_matrix.copy()
            PM_tokenizer = tokenizer.tokenizer
            row_index = 0
            fol_graph_indices = []
            ...
            if graph_matrix.shape[0] != len(fol_graph_indices):
                ...
                assert False
            if len(fol_graph_indices) >= max_length:
                graph_matrix = graph_matrix[len(fol_graph_indices)-max_length:,len(fol_graph_indices)-max_length:]
                fol_graph_indices = torch.tensor(fol_graph_indices[:max_length])
            else:
                graph_matrix = np.vstack([graph_matrix, np.zeros((max_length-len(fol_graph_indices), graph_matrix.shape[1]))])
                graph_matrix = np.hstack([graph_matrix, np.zeros((graph_matrix.shape[0], max_length-len(fol_graph_indices)))])
            ...
            edge_index,edge_type = adjacency_matrix_to_edge_index_type(torch.tensor(graph_matrix))
            return edge_index,edge_type,fol_graph_indices
            # edge_index = []
            # edge_type = []
            # word_nodes = graph.nodes
        ...
        def deal_data(sign_fname):
            ...
            with open(sign_fname,'r',encoding='utf-8') as f:
                ...
                for index,line in enumerate(lines):
                    ...
                    edge_index_gpt,edge_type_gpt,word_nodes_gpt = Rgraph_PreModel(BG.run_build(line['Ans1']))
                    edge_index_claude,edge_type_claude,word_nodes_claude = Rgraph_PreModel(BG.run_build(line['Ans2']))
                    edge_index_lla,edge_type_lla,word_nodes_lla = Rgraph_PreModel(BG.run_build(line['Ans3']))
                    ...
                    if 'bart' in opt.model_name or 'bert' in opt.model_name:
                        ...
                        # assert edge_type_gpt.shape[0] == 20 == edge_type_claude.shape[0] == edge_type_lla.shape[0]
                        # print(edge_type_gpt.shape,edge_type_claude.shape,edge_type_lla.shape)
                        ...
                        if 'bert' in opt.pretrained_model_name and 'roberta' not in opt.pretrained_model_name:
                            data = {
                                ...
                                'graph_index': torch.tensor([edge_index_gpt.tolist(),edge_index_claude.tolist(),edge_index_lla.tolist()]).long(),
                                'graph_type': torch.tensor([edge_type_gpt.tolist(),edge_type_claude.tolist(),edge_type_lla.tolist()]).long(),
